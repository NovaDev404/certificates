...
def update_recommended_cert(lines, certificates):
    """Update the recommended certificate section dynamically for whatever cert is present."""
    for i, line in enumerate(lines):
        if 'Recommend Certificate' in line and i + 1 < len(lines):
            next_line = lines[i + 1].strip()

            # Pick the first certificate in the list (or you could add smarter logic)
            if certificates:
                cert = certificates[0]  # first cert in your table
                status = cert.get('status', '').lower()
                company_name = cert.get('company', 'Unknown Company')

                if status == 'valid':
                    lines[i + 1] = f"**{company_name} - ✅ Signed**"
                elif status == 'revoked':
                    lines[i + 1] = f"**{company_name} - ❌ Revoked**"
                else:
                    lines[i + 1] = f"**{company_name} - ⚠️ Unknown**"
    return lines
def main():
    # Read README.md
